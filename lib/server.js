 ...
 *     <li><b>debugRejection</b> - if set to true, print out messages about rejections</li>
...
function SMTPServer(options) {
    ...
    this.options.ignoreEmptyFromInEnvelope = !! this.options.ignoreEmptyFromInEnvelope;
...
SMTPServerConnection.prototype.logRejection = function(smtpError, msg) {
    if (!this.server.options.debugRejection)
        return;

    console.log('simplesmtp-rejection', smtpError, msg, this.envelope);
}
    ...
    switch ((command || '').toString().trim().toUpperCase()) {
        ...
        case 'EHLO':
            if (!this.server.options.disableEHLO) {
                ...
                this.logRejection('502 5.5.2 Error: command not recognized', 'EHLO command disabled');
        ...
        default:
            this.logRejection('502 5.5.2 Error: command not recognized', 'Unknown command: ' + command);
            this.client.send('502 5.5.2 Error: command not recognized');
    }
};
...
SMTPServerConnection.prototype._onCommandMAIL = function(mail) {
    ...
    if (!this.hostNameAppearsAs) {
        this.logRejection('503 5.5.1 Error: send HELO/EHLO first', 'No hostNameAppearsAs');
    ...
    if (this.server.options.requireAuthentication && !this.authentication.authenticated) {
        this.logRejection('530 5.5.1 Authentication Required');
        return this.client.send('530 5.5.1 Authentication Required');
    }

    if (this.envelope.from) {
        this.logRejection('503 5.5.1 Error: nested MAIL command', 'this.envelope.from defined _onCommandMail');
        return this.client.send('503 5.5.1 Error: nested MAIL command');
    }

    if (!(match = mail.match(/^from\:\s*<([^@>]+\@([^@>]+))>(\s|$)/i)) && !(mail.match(/^from\:\s*<>/i))) {
        this.logRejection('501 5.1.7 Bad sender address syntax', 'No match for "from" field: ' + mail);
...
SMTPServerConnection.prototype._onCommandRCPT = function(mail) {
    ...
    if (!this.envelope.from) {
        if (!this.server.options.ignoreEmptyFromInEnvelope) {
          this.logRejection('503 5.5.1 Error: need MAIL command', 'No "from" field in envelope');
          return this.client.send('503 5.5.1 Error: need MAIL command');
        }
        else
          console.log('simple-smtp', 'ignore empty FROM in envelope: ', this.envelope);
    }

    if (!(match = mail.match(/^to\:\s*<([^@>]+\@([^@>]+))>$/i))) {
        this.logRejection('501 5.1.7 Bad recipient address syntax', 'No match for "to" field: ' + mail);
...
SMTPServerConnection.prototype._onCommandDATA = function() {
    ...
    if (!this.envelope.to.length) {
        this.logRejection('503 5.5.1 Error: need RCPT command', 'Undefined "to" field length');
...
SMTPServerConnection.prototype._onCommandAUTH = function(payload) {
    ...
    if (!this.server.options.requireAuthentication && !this.server.options.enableAuthentication) {
        this.logRejection('503 5.5.1 Error: authentication not enabled');
    ...
    if (!this.server.options.ignoreTLS && !this.client.secureConnection) {
        this.logRejection('530 5.7.0 Must issue a STARTTLS command first');
        return this.client.send('530 5.7.0 Must issue a STARTTLS command first');
    }

    if (this.authentication.authenticated) {
        this.logRejection('503 5.7.0 No identity changes permitted');
    ...
    if (this.server.options.authMethods.indexOf(method) < 0) {
        this.logRejection('535 5.7.8 Error: authentication failed: no mechanism available');
...
SMTPServerConnection.prototype._onCommandSTARTTLS = function() {
    if (this.client.secureConnection) {
        this.logRejection('554 5.5.1 Error: TLS already active');
...
SMTPServerConnection.prototype._onCommandHELO = function(host) {
    if (!host) {
        this.logRejection('501 Syntax: EHLO hostname', 'Missing host');
...
SMTPServerConnection.prototype._handleAuthPlain = function(payload) {
    if (payload.length) {
        ...
        if (userdata.length != 3) {
            this.logRejection('500 5.5.2 Error: invalid userdata to decode');
        ...
        this.server.emit('authorizeUser',
            ...
            password, (function(err, success) {
                if (err || !success) {
                    ...
                    this.logRejection('535 5.7.8 Error: authentication failed: generic failure', 'Authentication error: ' + (err || 'success is undefined'));
...
SMTPServerConnection.prototype._handleAuthXOAuth2 = function(payload) {
    if (this.authentication.state == 'AUTHXOAUTH2') {
        ...
        this.authentication.username = false;
        this.authentication.state = 'NORMAL';
        this.logRejection('535 5.7.1 Username and Password not accepted');
...
SMTPServerConnection.prototype._onDataReady = function() {
    if (this.server.listeners('dataReady').length) {
        this.server.emit('dataReady', this.envelope, (function(err, code) {
            ...
            if (err) {
                this.logRejection(err && err.SMTPResponse || ('550 ' + (err && err.message || 'FAILED')));
